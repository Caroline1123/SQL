...
try {
    ...
    $stmt = $pdo->prepare('SELECT * FROM Clients WHERE lastName = "Perry";');
    $stmt->execute();
    ...
    if (isset($_POST["last_name"]) && isset($_POST["first_name"]) && isset($_POST["birthday"])) {
        ...
        if (($isCardChecked && $isCardNumberProvided) || (!$isCardChecked && !$isCardNumberProvided)) {
            ...
            try {
                ...
                $pdo = new PDO('mysql:host=localhost;dbname=colyseum;charset=utf8', 'root', '');
                $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stmt = $pdo->prepare('UPDATE Clients SET lastName=:last_name, firstName=:first_name, birthDate=:birthday , card=:card, cardNumber=:card_number WHERE lastName ="Perry"');
